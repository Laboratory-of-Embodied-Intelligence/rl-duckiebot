...
if __name__=="__main__":    
 # Initialize tensorboard
    ...
    parser.add_argument("--init_random", default=10000, type=int)  # How much purely random policy on start
    parser.add_argument("--seed", default=0, type=int)  # Sets Gym, PyTorch and Numpy seeds
    parser.add_argument("--max_timesteps", default=100000, type=float)  # Max time steps to run environment for
    ...
    model = DDPG_V2(policy = 'CustomDDPGPolicy',env = env, tensorboard_log = tensorboard_log, verbose=0,
                    gamma = args.discount,
                    nb_rollout_steps = args.init_random,
                    return_range = [-1,1],
